...
const Footer = () => {
  const navLinks = ["Services", "Work", "About", "Culture", "Meet The Risers"];
  const resourceLinks = ["Testimonials", "Blog", "Webinars", "Careers"];
  const locationLinks = [
    "Sheffield",
    "Manchester",
    "London",
  ...
  const socialLinks = [
    { icon: <FaFacebookF size={9} />, href: "#" },
    { icon: <FaTwitter size={9} />, href: "#" },
    { icon: <FaLinkedinIn size={9} />, href: "#" },
    { icon: <FaYoutube size={9} />, href: "#" },
    { icon: <FaTiktok size={9} />, href: "#" },
    { icon: <FaInstagram size={9} />, href: "#" },
  ];

  return (
    <footer className="bg-[#0e0e0e] text-white lg:pt-7 pt-8 pb-6 px-3 md:px-6 font-sans rounded-2xl border-b-2xl overflow-hidden">
      <div className="w-full mx-auto">
        {/* Top Section */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:pb-20 md:pb-16 pb-6">
          <div className="md:col-span-5">
            <h2 className="text-2xl font-bold mb-2.5">
              Stay updated with Rise news
            </h2>
            <div className="relative group max-w-xl mb-4">
              <input
                type="email"
                placeholder="Your Email Address"
                className="w-full bg-white/9 border border-transparent py-2.5 px-4 rounded-full text-gray-300 transition-all outline-none placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-white/15 focus:border-transparent"
              />
              <button className="absolute right-1 top-1/2 -translate-y-1/2 bg-[#b2f6e3] p-2.5 rounded-full text-black transition-all duration-500 group-hover:bg-white group-hover:rotate-[75deg]">
            ...
            <div className="flex gap-1 relative z-50">
              {socialLinks.map((social, index) => (
                <a
                  key={index}
                  href={social.href}
                  className="flex items-center justify-center px-2 py-1 bg-white text-black rounded-[100px] hover:rounded-[2px] transition-all duration-300 ease-in-out cursor-pointer"
                  style={{ display: "flex" }}
                >
                  {social.icon}
                  <FiArrowUpRight size={10} className="ml-0.5" />
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="md:col-span-7 grid grid-cols-2 md:grid-cols-3 gap-8">
            <div>
              <ul className="space-y-1 border-l-1 border-white/20">
                {navLinks.map((link) => (
                  <li
                    key={link}
                    className="relative overflow-hidden h-5 group cursor-pointer ml-2"
                  >
                    <span className="block transition-transform duration-500 group-hover:-translate-y-full">
                      {link}
                    </span>
                    <span className="text-[#b2f6e3] absolute top-0 left-0 transition-transform duration-500 translate-y-full group-hover:translate-y-0">
                      {link}
                    </span>
                  </li>
                ))}
        ...
        {/* Bottom Section */}
        <div className="flex flex-col md:flex-row justify-between lg:items-start md:items-center text-[10px] text-white font-thin">
          <div className="flex flex-wrap justify-start  items-center md:max-w-md lg:max-w-3xl w-full">
            <span className="font-thin text-white">
              © 2025 Rise at Seven Ltd. All rights reserved
            </span>
            <Dot className="w-4 h-4 text-white mx-1" />
            <span className="font-thin text-white">
            ...
            <div className="flex items-center">
              <span className="relative cursor-pointer group font-thin text-white">
                Privacy Policy
          ...
          <div className="mt-1 md:mt-0">Website MadeByShape</div>
        </div>
      </div>
    </footer>
  );
