...
const AnimatedLink = ({ name, className = "" }) => (
  <span
    className={`hover-link-wrapper header-hover-link-wrapper cursor-pointer ${className}`}
  >
  ...
  return (
    <div className="relative z-[100]">
      ...
      <header
        ...
        ${
          scrolled
            ? "top-4 mx-auto w-[95%] bg-white/80 backdrop-blur-xl border border-white/20 rounded-full py-0 shadow-2xl"
      ...
      >
        <div className="lg:container w-full  mx-auto flex items-center justify-between">
          {/* Logo Section */}
          <div
            className={`w-32 transition-colors duration-300 ${scrolled ? "text-black" : "text-white"}`}
          >
            <svg
          ...
          <nav
            className={`hidden lg:flex items-center space-x-2 text-[13px] font-medium tracking-tight transition-colors duration-300 ${scrolled ? "text-black" : "text-white"}`}
          >
            {menuItems.map((item) => (
              <div
                key={item.name}
              ...
              >
                <div className="flex items-center gap-1">
                  <span
                    className={`px-3  transition-all rounded-full ${
                      scrolled
                  ...
                  {item.hasBadge && (
                    <span className="relative -top-[6px] -left-[2px] inline-flex items-center justify-center w-[18px] h-[18px] rounded-full bg-[#b2f9e1] text-[10px] font-black text-black leading-none">
                      {item.badgeCount}
                    </span>
                  )}
                </div>
                ...
                {item.isMega && (
                  <div
                    className={`absolute left-1/2 -translate-x-1/2 top-full mt-2 w-[850px] bg-white rounded-[32px] p-8 shadow-2xl text-black transition-all duration-300 transform origin-top ${
                      isServicesHovered
                  ...
                  >
                    <div className="grid grid-cols-12 gap-8 text-left">
                      <div className="col-span-8">
                        <p className="text-xs uppercase font-bold text-gray-400 mb-6">
                          Core Services
                        </p>
                      ...
                      <div className="col-span-4 relative rounded-2xl overflow-hidden bg-gray-100 flex flex-col justify-end p-5 group/card">
                        ...
                        <div className="relative z-10 text-center w-full">
                          ...
                          <button className="bg-black text-white text-[10px] font-black px-4 pt-2 transition duration-500 rounded-full hover:rounded-md uppercase">
          ...
          <div className="flex items-center gap-4">
            {/* Desktop Button - Hidden on Mobile/Tab */}
            <button className="hidden lg:flex px-3 py-2 bg-white rounded-full hover:rounded-md text-[13px] font-normal items-center gap-2 transition-all duration-500 w-fit group text-black">
              <AnimatedLink name="Get In Touch ↗" />
            </button>

            {/* Mobile Toggle Button - Forced to the Right */}
            <button
              onClick={() => setIsMenuOpen(true)}
              className="lg:hidden flex flex-col items-end gap-1.5 p-2 ml-auto"
            >
              <div
                className={`w-8 h-[1px] transition-colors ${scrolled ? "bg-black" : "bg-white"}`}
              ></div>
              <div
                className={`w-5 h-[1px] transition-colors ${scrolled ? "bg-black" : "bg-white"}`}
              ></div>
            </button>
          </div>
        </div>
      ...
      <div
        className={`fixed inset-0 z-[200] p-1 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] transform ${
          isMenuOpen
      ...
      >
        ...
        <div className="w-full h-full bg-[#1a1c1e]/95 backdrop-blur-xl rounded-2xl flex flex-col p-4 overflow-y-auto border border-white/10 shadow-2xl">
          {/* Header Section */}
          ...
          <nav className="flex flex-col space-y-1">
            {[
              ...
              <div
                key={name}
                className="flex items-center justify-between py-3 border-b border-white/10 group cursor-pointer"
              >
                <Link
                  to=" "
                  className="text-2xl leading-none font-medium text-white tracking-tight hover:opacity-80 transition-opacity"
                >
                  {name}
                </Link>

                {/* Dropdown Icon (Matches image_e0f48c.png style) */}
                {[
                  "Services",
                  "Industries",
                  "International",
          ...
          <div className="mt-auto pt-3 pb-4">
            <button className="w-full bg-white text-black py-2 rounded-full font-normal text-sm flex items-center justify-center gap-2 hover:bg-gray-100 transition-colors">
